...
public class AbsentDaoImpl implements AbsentDao {
    ...
    public Integer save(Absent absent) {
        ...
        String sql = "insert into absent(building_id,dormitory_id,student_id,dormitory_admin_id,date,reason) values(?,?,?,?,?,?)";
        ...
        try {
            ...
            statement.setString(5, absent.getDate());
    ...
    public List<Absent> list() {
        Connection connection = JDBCUtil.getConnection();
        String sql = "select a.id,b.name,d.name,s.name,a.reason,da.name,a.date from dormitory d,student s,building b,absent a,dormitory_admin da where d.id = a.dormitory_id and s.id = a.student_id and b.id = a.building_id and da.id = a.dormitory_admin_id";
        PreparedStatement statement = null;
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                list.add(new Absent(id, buildingName, dormitoryName, studentName, dormitoryAdminName, Date, reason));
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        } finally {
